...
const schema = yup.object().shape({
	...
	Message: yup.string().required("Please enter your message").min(2, "The message must be at least 2 characters"),
});
...
function ContactForm() {
    ...
    return (
        <Form className="form-contact" onSubmit={handleSubmit(onSubmit)}>
            {submitted ? <ValidationSuccess><p>Message sent.</p></ValidationSuccess> : ""}
            ...
            <Form.Group controlId="Email">
                <Form.Control type="email" required placeholder="Email.." {...register("email")}/>
                {errors.email && <ValidationError>{errors.email.message}</ValidationError>}
            </Form.Group>

            <Form.Group controlId="Message">
                <Form.Control as="textarea" required placeholder="Message.." rows={3} {...register("Message")}/>
